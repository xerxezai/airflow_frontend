...
const PAGE_CONFIG = {
  ...
  subtitle:
    "AI-assisted Input/Output schedule built from your Instrument Index",
  aiBadge: "AI-Powered",
...
const LAYOUT_CONFIG = {
  ...
  heroBlurClass:
    "absolute inset-x-0 top-0 h-full pointer-events-none blur-3xl opacity-50",
};
...
const INSTRUMENT_INDEX_REPORT_CONFIG = {
  ...
  emptyHint:
    "Run extraction from a P&ID PDF (Stage 1) to populate this report.",
...
const PID_EXTRACT_CONFIG = {
  ...
  metaFields: [
    ...
    {
      key: "drawing_title",
      label: "Drawing Title (optional)",
      placeholder: "e.g. LP Steam Generator",
      required: false,
    },
  ],
};
...
const WIZARD_STAGES = [
  {
    ...
    subtitle:
      "Drop the P&ID PDF (and optional Legend PDF) — the AI-vision extractor builds the instrument register for you.",
    icon: CpuChipIcon,
  ...
  },
  {
    ...
    subtitle:
      "Already have an Instrument Index workbook (or JSON payload)? Drop it here to feed the IO routing engine directly.",
  ...
  },
  {
    ...
    subtitle:
      "Set the project context, then generate and download the IO List.",
    icon: SparklesIcon,
...
function StatCard({ label, value, accent = "purple" }) {
  ...
  return (
    <div
      className={`rounded-xl px-4 py-3 bg-gradient-to-br ${ring} border border-white/60 shadow-sm`}
    >
...
function ProjectsShell({ onOpen }) {
  ...
  const [form, setForm] = useState(() =>
    Object.fromEntries(
      IO_LIST_PROJECT_FIELDS.map((f) => [f.key, f.defaultValue || ""]),
    ),
  );

  const refresh = (q = search) =>
    setProjects(listIoListProjects({ search: q }));
  ...
  return (
    <section
      className={`${LAYOUT_CONFIG.contentMaxW} ${LAYOUT_CONFIG.cardGapY}`}
    >
      <div className="rounded-2xl bg-white border border-gray-200 shadow-lg overflow-hidden">
        <div
          className={`px-6 py-5 bg-gradient-to-r ${PAGE_CONFIG.brandGradient} text-white flex items-center gap-4`}
        >
          ...
          <div className="flex-1">
            <h2 className="text-lg font-bold">
              {IO_LIST_PROJECT_COPY.panelTitle}
            </h2>
            <p className="text-xs text-purple-100 mt-0.5">
              {IO_LIST_PROJECT_COPY.panelSubtitle}
            </p>
          </div>
          <button
            type="button"
          ...
          {projects.length === 0 ? (
            <div className="text-center py-10">
              ...
              <h3 className="mt-4 text-sm font-semibold text-gray-800">
                {IO_LIST_PROJECT_COPY.emptyHeading}
              </h3>
              <p className="mt-1 text-xs text-gray-500 max-w-md mx-auto">
                {IO_LIST_PROJECT_COPY.emptyBody}
              </p>
              <button
                type="button"
                onClick={() => {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {projects.map((p) => {
                const st =
                  IO_LIST_PROJECT_STATUSES[p.status] ||
                  IO_LIST_PROJECT_STATUSES.draft;
                const cat =
                  IO_LIST_PROJECT_CATEGORIES.find((c) => c.id === p.category) ||
                  IO_LIST_PROJECT_CATEGORIES[0];
                return (
                  <div
                    key={p.id}
                  ...
                  >
                    ...
                    {cat && (
                      <span
                        className={`mt-2 self-start inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border ${cat.badgeBg} ${cat.badgeText} ${cat.badgeBorder}`}
                      >
                        <span>{cat.icon}</span>
                        {cat.label}
                      </span>
      ...
      {showCreate && (
        ...
        >
          ...
          >
            <div
              className={`px-5 py-4 bg-gradient-to-r ${PAGE_CONFIG.brandGradient} text-white flex items-center justify-between`}
            >
              <div className="flex items-center gap-2">
                <FolderPlusIcon className="h-5 w-5" />
                <h3 className="text-sm font-bold">
                  {editing
                    ? "Edit IO List Project"
                    : IO_LIST_PROJECT_COPY.createButton}
                </h3>
              </div>
              <button
            ...
            <div className="px-5 py-3 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-2">
              ...
              <button
                type="submit"
                className={`px-4 py-2 rounded-lg text-sm font-semibold text-white shadow bg-gradient-to-r ${PAGE_CONFIG.brandGradient} hover:shadow-md transition`}
              >
        ...
        >
          ...
          >
            <div className="px-5 py-4 bg-red-600 text-white">
              <h3 className="text-sm font-bold">
                {IO_LIST_PROJECT_COPY.deleteConfirmTitle}
              </h3>
            </div>
            ...
            <div className="px-5 py-3 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-2">
              ...
              <button
                type="button"
                onClick={doDelete}
                className="px-4 py-2 rounded-lg text-sm font-semibold text-white shadow bg-red-600 hover:bg-red-700 transition"
              >
                Delete
              </button>
            </div>
...
export default function IOListPage() {
  ...
  const dcsCount = useMemo(
    () =>
      generated.filter((r) => String(r.system || "").toUpperCase() === "DCS")
        .length,
    [generated],
  );
  const esdCount = useMemo(
    () =>
      generated.filter((r) => String(r.system || "").toUpperCase() === "ESD")
        .length,
    [generated],
  );
    ...
    try {
      ...
      if (activeProject) {
        try {
          ...
          console.warn("[IOList] snapshot save failed:", e);
        }
      }
      toast.success(
        `Generated ${rows.length} IO row(s) from ${instruments.length} instrument(s).`,
      );
    } catch (err) {
  ...
  const runFromJson = useCallback(() => {
    ...
    try {
      ...
      const instruments = Array.isArray(parsed)
        ? parsed
        : parsed?.instruments || [];
      if (!instruments.length) {
  ...
  const runFromPid = useCallback(async () => {
    ...
    const abort = setTimeout(
      () => ctrl.abort(),
      PID_EXTRACT_CONFIG.uploadTimeoutMs,
    );
    ...
    try {
      ...
      fd.append(
        "drawing_number",
        pidMeta.drawing_number ||
          pidNo ||
          pidPdfFile.name.replace(/\.pdf$/i, ""),
      );
      ...
      while (true) {
        try {
          ...
          if (
            attempt < PID_EXTRACT_CONFIG.maxRetries &&
            isTransientNetworkError(netErr)
          ) {
            attempt += 1;
            const delay =
              PID_EXTRACT_CONFIG.retryBackoffBaseMs *
              Math.pow(PID_EXTRACT_CONFIG.retryBackoffFactor, attempt - 1);
            setExtractStatus(
              `Network blip — retrying (${attempt}/${PID_EXTRACT_CONFIG.maxRetries})…`,
            );
      ...
      const instruments = Array.isArray(data?.instruments)
        ? data.instruments
        : Array.isArray(data?.results?.instruments)
      ...
      setExtractStatus(
        `Extraction complete — building IO List from ${instruments.length} instruments…`,
      );
      const rows = buildIoListRowsFromInstruments(instruments, {
      ...
      toast.success(
        `Generated ${rows.length} IO row(s) from ${instruments.length} extracted instrument(s).`,
      );
      setExtractStatus("");
    } catch (err) {
      clearTimeout(abort);
  ...
  return (
    <div className={LAYOUT_CONFIG.wrapper}>
      ...
      <section className="relative overflow-hidden">
        ...
        />
        <div
          className={`relative ${LAYOUT_CONFIG.contentMaxW} ${LAYOUT_CONFIG.heroPaddingY}`}
        >
          {activeProject && (
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              {(() => {
                const cat = IO_LIST_PROJECT_CATEGORIES.find(
                  (c) => c.id === activeProject.category,
                );
                return cat ? (
                  <span
                    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold border ${cat.badgeBg} ${cat.badgeText} ${cat.badgeBorder}`}
                  >
                    <span>{cat.icon}</span>
                    {cat.label}
                  </span>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-2 text-2xl sm:text-3xl font-bold tracking-tight text-gray-900">
                <span
                  className={`bg-gradient-to-r ${PAGE_CONFIG.brandGradient} bg-clip-text text-transparent`}
                >
                  {PAGE_CONFIG.title}
                </span>
              </h1>
              <p className="mt-1.5 text-sm text-gray-600 max-w-2xl">
                {PAGE_CONFIG.subtitle}. Drop in an Instrument Index extraction,
                let the engine classify each tag (DCS / ESD, AI / AO / DI / DO,
                IS / NIS, signal type, wiring, voltage) and download the
                ADNOC-template workbook in seconds.
              </p>
      ...
      {/* ─── Feature highlights ─────────────────────────────────────────── */}
      <section
        className={`${LAYOUT_CONFIG.contentMaxW} ${LAYOUT_CONFIG.sectionGapY}`}
      >
          ...
          <div className="rounded-2xl bg-white border border-gray-200 shadow-lg overflow-hidden">
            ...
            <div className="px-6 pt-5 pb-2">
              <ol className="flex items-center gap-2 sm:gap-4">
                {WIZARD_STAGES.map(({ id, label, icon: StageIcon }, idx) => {
                  ...
                  const isReachable =
                    id <= stage || stageComplete[id - 1] || id === 2;
                  return (
                    <li
                      key={id}
                    ...
                    >
                      ...
                      >
                        <span
                          className={`flex items-center justify-center h-6 w-6 rounded-full text-[11px] font-bold flex-shrink-0 ${
                            isActive
                              ? "bg-white/20"
                              : isComplete
                                ? "bg-emerald-100"
                      ...
                      {idx < WIZARD_STAGES.length - 1 && (
                        <span
                          className="hidden sm:block h-px flex-1 bg-gray-200"
                          aria-hidden
                        />
                      )}
                    </li>
            ...
            {stage === 1 && (
              <div className="px-6 pt-4 pb-6">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-600 mt-0.5">
                        Drop the P&amp;ID PDF (and optional Legend PDF). We run
                        the same AI-vision extractor used by{" "}
                        <Link
                          to={PAGE_CONFIG.instrumentIndexLink}
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                    <label className="block">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          setPidPdfFile(e.target.files?.[0] || null);
                          setGenerated([]);
                          setSourceCount(0);
                        }}
                      ...
                      {pidPdfFile && (
                        <div className="mt-1.5 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-white border border-fuchsia-200 text-[11px] text-gray-700">
                          ...
                          <span className="text-gray-500">
                            ({Math.round(pidPdfFile.size / 1024)} KB)
                          </span>
                        </div>
                      )}
                    </label>
                    <label className="block">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setLegendPdfFile(e.target.files?.[0] || null)
                        }
                ...
                <div className="mt-3 text-[11px] text-gray-500">
                  ...
                  and feed the engine with an existing Instrument Index
                  workbook.
                </div>
              </div>
            )}
            ...
            {stage === 2 && (
              <div className="px-6 pt-4 pb-6">
                ...
                {iolistMode === "file" ? (
                  <div className="rounded-xl border-2 border-dashed border-purple-200 bg-purple-50/40 p-6">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                      ...
                      <div className="flex-1">
                        <div className="text-sm font-semibold text-gray-800">
                          Upload the Instrument Index workbook
                        </div>
                        <p className="text-xs text-gray-600 mt-0.5">
                          Accepted formats:{" "}
                          ...
                          . The first sheet with data is auto-detected — headers
                          are flexible (tag_number / Tag No. / Tag, etc).
                        </p>
                      </div>
                      <input
                        type="file"
                  ...
                  <div className="rounded-xl border-2 border-dashed border-indigo-200 bg-indigo-50/40 p-6">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <div className="text-sm font-semibold text-gray-800">
                          Paste Instrument Index JSON
                        </div>
                        <p className="text-xs text-gray-600 mt-0.5">
                          Array of instrument objects, or{" "}
                          <span className="font-mono">
                            {"{ instruments: […] }"}
                          </span>
                          .
                ...
                <div className="mt-3 text-[11px] text-gray-500">
                  Already extracted the register in Stage 1? You can skip this
                  and go straight to{" "}
                  <button
                    type="button"
                    onClick={() => setStage(3)}
              ...
              <div className="px-6 pt-4 pb-6">
                  ...
                  <label className="flex items-center gap-3 px-3 py-2 rounded-lg border border-gray-200 bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors">
                    ...
                    <div className="text-xs">
                      <div className="font-semibold text-gray-800">
                        Include HMI indicators
                      </div>
                      <div className="text-gray-500">
                        FI / PI / TI… (off by default)
                      </div>
                    </div>
                  </label>
                </div>
                ...
                <div className="mt-4 rounded-lg bg-gray-50 border border-gray-200 px-3 py-2 text-xs text-gray-700">
                  ...
                  {resolvedSource === "file" ? (
                    ...
                    <>
                      P&amp;ID PDF —{" "}
                      <span className="font-mono">{pidPdfFile?.name}</span> (AI
                      extract)
                    </>
                  ) : (
                    <span className="text-amber-700">
                      No source provided yet. Go back to Stage 1 or Stage 2 to
                      add one.
                    </span>
                  )}
                </div>
                ...
                {stage < WIZARD_STAGES.length ? (
                  <>
                    <button
                      type="button"
                      onClick={() =>
                        setStage((s) => Math.min(WIZARD_STAGES.length, s + 1))
                      }
            ...
            {pidIndexResult &&
              Array.isArray(pidIndexResult.instruments) &&
              pidIndexResult.instruments.length > 0 && (
                <div className="px-6 pb-6">
                  <div className="flex items-center gap-2 mb-3 flex-wrap">
                    ...
                    <span className="text-[11px] text-gray-500">
                      — {INSTRUMENT_INDEX_REPORT_CONFIG.subtitle}
                    </span>
                    <span className="ml-auto inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-violet-50 text-violet-700 border border-violet-200">
                      <CheckBadgeIcon className="h-3.5 w-3.5" />
                      {pidIndexResult.total ??
                        pidIndexResult.instruments.length}{" "}
                      instruments
                    </span>
                    {pidIndexResult.excel_url && (
                      <button
                        ...
                        onClick={() => {
                          ...
                          const safeName = (
                            pidIndexResult?.drawing_info?.drawing_number ||
                            pidNo ||
                            "export"
                  ...
                  {pidIndexResult.category_summary &&
                    Object.keys(pidIndexResult.category_summary).length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-3">
                        {Object.entries(pidIndexResult.category_summary)
                          .sort()
                          .map(([cat, count]) => {
                            const cls =
                              INSTRUMENT_INDEX_REPORT_CONFIG.categoryColors[
                                cat
                              ] ||
                              INSTRUMENT_INDEX_REPORT_CONFIG.categoryColors
                                .default;
                            return (
                              ...
                              >
                                {cat}
                                <span className="ml-1 px-1.5 py-0.5 rounded-full bg-white/70 text-[10px] font-bold">
                                  {count}
                                </span>
                              </span>
                            );
                    ...
                    <table className="min-w-full text-xs">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {pidIndexResult.instruments
                          .slice(
                            0,
                            INSTRUMENT_INDEX_REPORT_CONFIG.maxPreviewRows,
                          )
                          .map((inst, i) => {
                            ...
                            return (
                              <tr
                                key={`${inst.tag_number || "tag"}-${i}`}
                                className="hover:bg-violet-50/40 transition-colors"
                              >
                                {INSTRUMENT_INDEX_REPORT_CONFIG.columns.map(
                                  (c) => {
                                    const raw =
                                      c.key === "index_no"
                                        ? (inst.index_no ?? i + 1)
                                    ...
                                    if (c.key === "category") {
                                      return (
                                        ...
                                        >
                                          <span
                                            className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-semibold border ${catCls}`}
                                          >
                                            {String(raw || "—")}
                                          </span>
                                        </td>
                                    ...
                                    return (
                                      <td
                                        key={c.key}
                                        className="px-2.5 py-1.5 whitespace-nowrap text-gray-700"
                                      >
                                        {String(raw ?? "")}
                                      </td>
                                    );
                  ...
                  {pidIndexResult.instruments.length >
                    ...
                    <p className="mt-2 text-[11px] text-gray-500 italic">
                      Showing first{" "}
                      {INSTRUMENT_INDEX_REPORT_CONFIG.maxPreviewRows} of{" "}
                      {pidIndexResult.instruments.length} rows — full set is in
                      the Excel download.
                    </p>
                  )}
                </div>
            ...
            {generated.length > 0 && (
              <div className="px-6 pb-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-sm font-bold text-gray-800">
                    Preview — first {previewRows.length} of {generated.length}{" "}
                    row(s)
                  </h3>
                  <span className="ml-auto text-[11px] text-gray-500">
                    Showing {PREVIEW_COLUMN_COUNT} of {IO_LIST_COLUMNS.length}{" "}
                    columns — full schema lands in the download.
                  </span>
                </div>
                <div className="overflow-x-auto rounded-xl border border-gray-200 shadow-sm">
                  <table className="min-w-full text-xs">
                    <thead className="bg-gradient-to-r from-purple-50 to-indigo-50 text-gray-700">
                      <tr>
                        {IO_LIST_COLUMNS.slice(0, PREVIEW_COLUMN_COUNT).map(
                          (c) => (
                            <th
                              key={c.key}
